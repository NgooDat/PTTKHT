...
@WebServlet(name = "bcs_check", urlPatterns = {"/bcs-check"})
public class bcs_check extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        dispatcher = request.getRequestDispatcher("views/bcs/bcs_tieuchi.jsp");
        dispatcher.forward(request, response);
    }
                    ...
                    if (dsDRL.get(0).getStatus() == 5) {
                        ...
                        if (request.getParameter("iddsave") != null) {
                            for (int i = 0; i < dsDRL.size(); i++) {
                                ...
                                k.setStatus(2);
                                new DRL_dao().updateDiemSV(k.getId(), diem);
                                new DRL_dao().updateStatus(2, k.getSinhVienID(), hkk.getId());
